...
def get_gene(input_set, data_set):
	...
	result_1['snp_gap_id'] = snp_input['snp_gap_id']

	for i in range(len(snp_input['chr'])):
		for j in range(len(data_input['chr'])):
			if snp_input.chr_pos[i] > data_input.midpoint[j]:
				if j == (len(data_input['chr']) - 1):
					dis1 = snp_input.chr_pos[i] - data_input.midpoint[(j-1)]
					if dis1 < data_input.radius[(j-1)]:
						result_1.gene_name[i] = data_input.gene_name[(j-1)]
						result_1.score[i] = 100
						break
					else:
						score1 = (data_input.radius[(j-1)] / dis1)*100
						result_1.gene_name[i] = data_input.gene_name[(j-1)]
			...
			if j == 0:
				dis2 = data_input.midpoint[j] - snp_input.chr_pos[i]
				if dis2 < data_input.radius[j]:
					result_1.gene_name[i] = data_input.gene_name[j]
					result_1.score[i] = 100
					break
				else:
					score2 = (data_input.radius[j] / dis2)*100
					result_1.gene_name[i] = data_input.gene_name[j]
...
export_csv = final.to_csv('result_para.csv', index = None, header = True)